...
if backpointer
    for iNode = 1:length(graphVector)
        if ~isempty(graphVector(iNode).backpointer)
            ...
            quiver3(x,y,z,u,v,w,'g','LineWidth',1.5,'AutoScale','off')
        end
    end
end
...
if ~isempty(endPoints)
    hold on
    plot3(endPoints(1,1),endPoints(2,1),endPoints(3,1),'g.','MarkerSize',20)
    plot3(endPoints(1,2),endPoints(2,2),endPoints(3,2),'r.','MarkerSize',20)
end
